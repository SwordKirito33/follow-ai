...
const SocialLogin: React.FC<SocialLoginProps> = ({ onSuccess, onError }) => {
  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    try {
      ...
      toast.error(`${provider} Login Failed`, message);
      onError?.(message);
    } finally {
      setLoading(null);
    }
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-4 py-2.5 border border-white/20 dark:border-gray-600 rounded-lg hover:bg-gray-800/5 dark:hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading === 'google' ? (
