  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-white/10 dark:border-gray-800">
  ...
  if (item.children) {
    return (
      <div ref={dropdownRef} className="relative">
        <button
          ...
          className={cn(
            ...
            isActive
              ...
              : 'text-gray-400 dark:text-gray-400 hover:text-white dark:hover:text-white hover:bg-white/10 dark:hover:bg-gray-800'
          )}
        >
          {item.icon}
          {item.label}
        ...
        {dropdownOpen && (
          <div className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-white/10 dark:border-gray-700 py-2 z-50">
            {item.children.map((child, index) => (
              <Link key={index} href={child.href || '#'}>
                ...
                >
                  ...
                  {child.label}
                  {child.badge && (
                    <span className="ml-auto px-2 py-0.5 text-xs font-medium bg-primary-purple/20 dark:bg-purple-900/30 text-primary-purple dark:text-purple-400 rounded-full">
                      {child.badge}
                    </span>
                  )}
                </a>
  ...
  if (item.onClick) {
    return (
      ...
      >
        ...
        {item.label}
        {item.badge && (
          <span className="px-2 py-0.5 text-xs font-medium bg-primary-purple/20 dark:bg-purple-900/30 text-primary-purple dark:text-purple-400 rounded-full">
            {item.badge}
          </span>
        )}
      </button>
...
export function Sidebar({
  ...
  return (
    <aside
      className={cn(
        'h-screen bg-white dark:bg-gray-900 border-r border-white/10 dark:border-gray-800 flex flex-col transition-all duration-300',
        collapsed ? 'w-16' : 'w-64',
        className
      )}
    >
      {/* Logo */}
      <div className="h-16 flex items-center justify-between px-4 border-b border-white/10 dark:border-gray-800">
        {!collapsed && logo}
        {onCollapse && (
          <button
            onClick={() => onCollapse(!collapsed)}
            className="p-2 rounded-lg text-gray-400 hover:text-gray-400 dark:hover:text-gray-300 hover:bg-white/10 dark:hover:bg-gray-800"
          >
            <svg
      ...
      {items.map((item, index) => (
        <React.Fragment key={index}>
          ...
          {item.href ? (
            ...
            <span className="text-white dark:text-white font-medium">{item.label}</span>
          )}
        </React.Fragment>
      ))}
    </nav>
