    ...
    <div className={cn('overflow-hidden rounded-lg border border-white/10 dark:border-gray-700', className)}>
      <div className="overflow-x-auto">
        <table className="w-full">
          {/* Header */}
          <thead className="bg-gray-800/5 dark:bg-gray-800">
            <tr>
              {selectable && (
            ...
            {loading ? (
                ...
                <tr key={index}>
                  {selectable && (
                    <td className={cn(compact ? 'px-3 py-2' : 'px-4 py-3')}>
                      <div className="w-4 h-4 bg-gray-800/10 dark:bg-gray-700 rounded animate-pulse" />
                    </td>
                  )}
                  {columns.map((column) => (
                    <td key={String(column.key)} className={cn(compact ? 'px-3 py-2' : 'px-4 py-3')}>
                      <div className="h-4 bg-gray-800/10 dark:bg-gray-700 rounded animate-pulse" />
                    </td>
                  ))}
                </tr>
              ))
              ...
              sortedData.map((row, index) => (
                <tr
                  ...
                  className={cn(
                    'transition-colors',
                    striped && index % 2 === 1 && 'bg-gray-800/5 dark:bg-gray-800/50',
                    hoverable && 'hover:bg-gray-800/5 dark:hover:bg-gray-800',
...
export function BadgeCell({ value, variant = 'default' }: BadgeCellProps) {
  const variantClasses = {
    default: 'bg-gray-800/10 dark:bg-gray-800 text-gray-300 dark:text-gray-300',
...
export function ProgressCell({ value, max = 100, showLabel = true }: ProgressCellProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-2 bg-gray-800/10 dark:bg-gray-800 rounded-full overflow-hidden">
        <div
