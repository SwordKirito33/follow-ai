      ...
      <div className="container mx-auto max-w-6xl grid lg:grid-cols-[2fr_1fr] gap-8 relative z-10">
        <div>
          <header className="mb-8 animate-slideDown">
            <h1 className="text-4xl sm:text-5xl font-black text-white mb-2 flex items-center gap-3 tracking-tight">
              <span className="text-4xl animate-float">📰</span> <span className="gradient-text">{t('news.title')}</span>
            </h1>
            <p className="text-lg text-gray-400 font-medium">{t('news.subtitle')}</p>
          </header>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all transform hover:scale-105 ${
                  activeTab === tab.key
                    ...
                    : 'glass-card text-gray-300 hover:bg-white/90'
                }`}
              >
                {tab.label}
              </button>
            ...
            {filtered.map((item, idx) => (
              <article key={item.id} className="glass-card rounded-xl shadow-xl p-5 flex gap-4 group hover:shadow-2xl transition-all duration-300 card-3d animate-slideUp" style={{ animationDelay: `${idx * 0.05}s` }}>
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide mb-2">
                  <span className={`px-2 py-1 rounded ${item.type === 'launch' ? 'bg-primary-purple/20 text-purple-700' : item.type === 'update' ? 'bg-amber-100 text-amber-800' : item.type === 'trending' ? 'bg-accent-green/20 text-green-800' : 'bg-primary-blue/20 text-blue-800'}`}>
                    {item.type}
                  </span>
        ...
        <aside className="space-y-8">
          <div className="glass-card rounded-xl shadow-xl p-5 animate-slideUp">
            ...
            <div className="space-y-2 text-sm text-gray-300">
              <div className="flex justify-between"><span># AI Coding</span><span className="text-accent-green font-semibold">+234%</span></div>
              <div className="flex justify-between"><span># Video Gen</span><span className="text-accent-green font-semibold">+180%</span></div>
              <div className="flex justify-between"><span># Voice AI</span><span className="text-accent-green font-semibold">+95%</span></div>
              <div className="flex justify-between"><span># Image Gen</span><span className="text-accent-green font-semibold">+67%</span></div>
            </div>
          </div>
